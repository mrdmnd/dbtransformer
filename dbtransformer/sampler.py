...
class RelationalSampler:
    ...
    def sample_context(
        ...
        def add_to_frontier(new_row: Row, from_row: Row, lt: str):
        ...
        while len(cells) < self.context_length and frontier:
            ...
            for parent in row.f2p_neighbors:
                if (parent.row_id not in visited and
                    parent.row_id not in to_visit and
                    (parent.timestamp is None or 
                     seed_row.timestamp is None or
                     parent.timestamp <= seed_row.timestamp)):
                    add_to_frontier(parent, row, "f2p")
            
            # Add P→F neighbors with temporal filtering and width bound
            valid_children = [
                child for child in row.p2f_neighbors
                if (child.row_id not in visited and
                    child.row_id not in to_visit and
                    (child.timestamp is None or 
                     seed_row.timestamp is None or
                     child.timestamp <= seed_row.timestamp))
            ]
            
            # Subsample children (width bound)
            sampled_children = random.sample(
                valid_children,
                min(self.width_bound, len(valid_children))
            )
    ...
    def _select_row(
        ...
        f2p_rows = [
            ...
            if link_type.get(row.row_id) == "f2p"
        ]
